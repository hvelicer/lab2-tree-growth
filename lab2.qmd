...
tree_dat %>%
  ...
  print()
```
*Answer:* The "Small" class contains 252 trees, the "Medium" class contains 1,963 trees, and the "Large" class contains 76 trees.
...
tree_dat <- tree_dat %>%
  ...
  ungroup()
growth_records <- tree_dat %>%
  group_by(treeID) %>%
  summarize(total_years = n(), avg_growth = mean(annual_growth, na.rm = TRUE)) %>%
  filter(total_years >= 10)
growth_stats <- tree_dat %>%
  inner_join(growth_records, by = "treeID")%>%
  group_by(species) %>%
  summarize(mean_growth = mean(avg_growth, na.rm = TRUE), sd_growth = sd(avg_growth, na.rm = TRUE)) %>%
...
slowest_species <- growth_stats %>%
  ...
  print()
```
The 3 species with the fastest growth are Pinus resinosa (*0.359, overall fastest*), Populus tremuloides (*0.331*), and Pinus banksiana (*0.326*). The 3 species with the slowest growth are Larix laricina (*0.150*), Thuja occidentalis (*0.152*), and Quercus rubra (*0.167*).

<img src="img/tree2.jpg" width="360" alt="Pinus resinosa">
<img src="img/tree1.jpg" width="425" alt="Pinus resinosa">
These images depict Pinus resinosa, commonly known as red pine, which is the tree with the overall fastest growth.